...
class ThoracicCXR(PyTorchDataset):
    def __init__(self, root, train=True, transform=None, target_transform=None):
        root = os.path.join(root, "vil-data")
        self.root = os.path.expanduser(root)
        self.train = train
        self.transform = transform
        self.target_transform = target_transform
        ...
        self.data = [patched_imagefolder(os.path.join(self.data_path, d), transform=self.transform) for d in
                     self.domains]
    ...
    def __getitem__(self, index):
        ...
        if self.data_is_list:
            ...
            raise NotImplementedError(
                "Cannot directly index the combined 'Dataset' object in VIL/DIL mode. Access domains via self.data[domain_index].")
